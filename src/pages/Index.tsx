...
import { createInitialGrid, resetGrid, START_NODE_ROW, START_NODE_COL, END_NODE_ROW, END_NODE_COL } from "@/utils/gridHelpers";
...
const Index = () => {
  ...
  const moveStartNode = (currentGrid: Cell[][], row: number, col: number): Cell[][] => {
    ...
    // Set new start (if not end or wall)
  ...
  const moveEndNode = (currentGrid: Cell[][], row: number, col: number): Cell[][] => {
    ...
    // Set new end (if not start or wall)
  ...
  const visualize = async () => {
    ...
    // Find start and end cells AFTER resetting grid
    ...
    // Small delay to show cleared state
      ...
      <div className="max-w-[1600px] mx-auto space-y-6">
        ...
        <div className="bg-card p-6 rounded-lg border border-border space-y-2">
          ...
          <ul className="text-sm text-muted-foreground space-y-1 list-disc list-inside">
            <li>Drag the start (red) and end (red) nodes to reposition them</li>
